...
public class MySqlSourceEnumerator implements SplitEnumerator<MySqlSplit, PendingSplitsState> {
    ...
    public void start() {
        ...
        /**
         * 打开 MySqlHybridSplitAssigner
         * 底层会执行切片逻辑
         */
        splitAssigner.open();

        /**
         * 可能需要暂定 binlog reader 也即向 SourceReader 发送 SuspendBinlogReaderEvent 事件
         */
        suspendBinlogReaderIfNeed();
        /**
         * 可能需要唤醒 binlog reader 也即向 SourceReader 发送 WakeupReaderEvent 事件
         */
    ...
    private void assignSplits() {
        /**
         * 获取哪些 SourceReader subtask 申请切片
         */
        ...
        while (awaitingReader.hasNext()) {
            ...
            /**
             * 从切片好的缓存拉取一个切片 MySqlSnapshotSplit 信息
             */
            Optional<MySqlSplit> split = splitAssigner.getNext();
            if (split.isPresent()) {
                final MySqlSplit mySqlSplit = split.get();
                /**
                 * 切片信息响应给 SourceReader subtask
                 */
                context.assignSplit(mySqlSplit, nextAwaiting);
                /**
                 * 移除 SourceReader subtask 被分配切片信息
                 */
                awaitingReader.remove();
                /**
                 * Assign split MySqlSnapshotSplit{
