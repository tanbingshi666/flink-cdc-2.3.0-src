...
public class MySqlSnapshotSplitReadTask
    ...
    protected SnapshotResult<MySqlOffsetContext> doExecute(
        ...
        /**
         * 执行 mysql 语句 SHOW MASTER STATUS 记录当前 binlog low watermark
         */
        /**
         * +------------------+----------+--------------+------------------+-------------------+
         * | File             | Position | Binlog_Do_DB | Binlog_Ignore_DB | Executed_Gtid_Set |
         * +------------------+----------+--------------+------------------+-------------------+
         * | mysql-bin.000046 |      154 | medical      |                  |                   |
         * +------------------+----------+--------------+------------------+-------------------+
         */
        final BinlogOffset lowWatermark = currentBinlogOffset(jdbcConnection);
        /**
         *  Snapshot step 1 - Determining low watermark {
         *  ts_sec=0,
         *  file=mysql-bin.000046,
         *  pos=154, kind=SPECIFIC,
         *  gtids=,
         *  row=0,
         *  event=0}
         *  for split
        ...
        /**
         * 读取 snapshot 数据
         * 本质上还是基于 JDBC 方式拼接 SQL 查询单个切片范围内的数据
         */
        createDataEvents(ctx, snapshotSplit.getTableId());

        /**
         * 执行 mysql 语句 SHOW MASTER STATUS 记录当前 binlog high watermark
         * 因为在读取切片范围数据的时候 可能这些数据已经发生 change
         * 所以需要从 low watermark 读取到 high watermark 的 binlog 数据与 SQL 查询
         * 得到的数据进行 merge
         */
        ...
        try (PreparedStatement selectStatement =
            ...
            while (rs.next()) {
                ...
                dispatcher.dispatchSnapshotEvent(
                        table.id(),
                        /**
                         * 创建 SnapshotChangeRecordEmitter 封装读取到的数据
                         */
