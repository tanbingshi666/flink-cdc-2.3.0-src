...
public class MySqlSnapshotSplitAssigner implements MySqlSplitAssigner {
    ...
    public void open() {
        /**
         * 打开 JdbcConnection 也即连接 mysql
         */
        chunkSplitter.open();
        /**
         * 根据配置查询 mysql 获取目标监控表
         * 缓存在 remainingTables 泛型为 TableId(库名 + 表名)
         */
    ...
    private void discoveryCaptureTables() {
        ...
        if (needToDiscoveryTables()) {
            ...
            try (JdbcConnection jdbc = openJdbcConnection(sourceConfig)) {
                /**
                 * 查询 mysql 获取指定监控哪些表 (TableId = 库名 + 表名)
                 */
    ...
    private void startAsynchronouslySplit() {
        /**
         * 之前已经把需要监控的 TableId 缓存在 remainingTables 故 remainingTables 不为空
         */
        if (chunkSplitter.hasNextChunk() || !remainingTables.isEmpty()) {
            /**
             * 创建大小为 1 的线程池用来 chunk 切片
             */
            if (executor == null) {
            ...
            /**
             * 执行 chunk 切片 也即遍历每个表
             * 根据表的行数 每个 chunk 默认大小为 8096 为一个切片
             * 将切片信息缓存在 remainingSplits 内存中
             */
